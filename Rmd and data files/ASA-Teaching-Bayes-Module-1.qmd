---
title: 'Module 1: Introducing Bayesian Reasoning'
...
**This module, which assumes no previous background in Bayesian statistics, will provide examples of how you can introduce Bayesian statistical reasoning in an *introductory* statistics class.**
...
We'll make an assessment of plausibility before collecting sample data (the **prior distribution**), and then revise after we have observed some data (the **posterior distribution**).
...
# Formulating a prior distribution (~10 minutes) {#sec-formulating-prior}
...
That is, which of the five statements in the bullet points above do you think is least plausible?
Continue to rank the five values from least to most plausible.


1. We have ranked the values from least to most plausible.
We'll now assess their relative plausibility by assigning "units of plausibility".
Suppose the value you think is least plausible represents 1 "unit of plausibility" (this just provides a baseline).
Which value --- 0.1, 0.3, 0.5, 0.7, 0.9 --- do you think is the next least plausible?
Roughly, how many *times* more plausible than the least plausible value do you think it is?
How many "units of plausibility" does the second least plausible represent?

    For example, suppose you think that 0.1 is the least plausible and 0.3 is the next least plausible value of $\theta$.
If you think that 0.3 is 2 times more plausible than 0.1, then you would assign 2 units of plausibility to 0.3.
(Roughly, you would be willing to engage in a bet that $\theta$ is 0.1 versus 0.3 at 2-to-1 odds.)


1. Continue in the manner of the previous part to assign units of plausibility to each of the values 0.1, 0.3, 0.5, 0.7, 0.9.
...
1. If 0.1, 0.3, 0.5, 0.7, 0.9 are the only possible values of $\theta$, then they should account for 100% of your total plausibility.
    ...
    Using your units of plausibility, what percent of your total plausibility does each of the values 0.1, 0.3, 0.5, 0.7, 0.9 represent?
...
::: {.callout-tip}
We certainly discuss issues relating to choice of prior and influence of the prior, but we recommend that you don't *overemphasize* issues relating to choice of prior.
The prior is just one of many assumptions.
Don’t spend too much time on choosing the prior.
It’s the data and the posterior that matters most!
:::
...
prior_table |>
  ggplot(aes(x = theta,
             y = prior)) +
  geom_point(col = bayes_col["prior"], size = 2) +
  geom_line(linetype = "dashed", col = bayes_col["prior"], linewidth = 1) +
...
# Simulating potential samples given $\theta$ (~5 minutes) {#sec-sampling-dist}
...
::: {.callout-note}
In the interest of time we are assuming some familiarity with simulation-based inference, but we would discuss simulations and the idea of sampling distributions in more detail with students.
We would also conduct some tactile simulations with dice or spinners first.
:::
...
#| fig-cap: "Sampling distribution of y for each possible value of theta"
#| ref.label: sampling-distributions
#| echo: false
...
# Reassessing plausibility (~5 minutes) {#sec-updating}

Recall our prior distribution from @sec-formulating-prior (Short solution).
...
1. $y = 18$: $\theta = 0.7$ had the highest prior plausibility, and has the highest likelihood of $y = 18$, so $\theta = 0.7$ has the highest posterior plausibility


1. $y = 15$: $\theta = 0.7$ and $\theta = 0.5$ have roughly the same likelihood of $y = 15$, but $\theta = 0.7$ had the higher prior plausibility, so $\theta = 0.7$ has the highest posterior plausibility


1. $y = 14$: $\theta = 0.7$ had the highest prior plausibility, but $\theta = 0.5$ has the highest likelihood of $y = 14$, so it's not obvious which will have the higher posterior plausibility.


We need a way to balance prior and likelihood, which we'll investigate below.
...
::: {.callout-note}
This discussion assumes some familiarity with a likelihood function (and maximum likelihood).
We would discuss these ideas in more detail with students, in non-technical terms (certainly no calculus).
:::
...
@fig-sampling-distributions shows that the likelihood of observing $y = 18$ is greatest when $\theta = 0.7$.
...
@fig-sampling-distributions shows that the likelihood of observing $y = 15$ is about the same when $\theta = 0.5$ as when $\theta = 0.7$, so $\theta = 0.5$ and $\theta = 0.7$ seem equally consistent with the sample data (and more consistent than the other $\theta$ values.)
...
# Revising our simulation to reflect prior plausibility (~10 minutes) {#sec-simulation}

In @sec-sampling-dist, we simulated values of $y$ for each of the possible $\theta$ values, but the values of $\theta$ were not equally plausible.

::: {.panel-tabset}

## Questions

1. How can we revise our simulation of $y$ values to reflect our prior assessment of plausible values of $\theta$?
Hint: consider a two-stage simulation.
(*We start with this broad question, but students might need some additional prodding.*)


1. The idea is to (1) *simulate a value of $\theta$ from the prior distribution*, then (2) given $\theta$ simulate $y$ as before.
Conduct a few repetitions of this simulation and [enter your results in this Google sheet](https://docs.google.com/spreadsheets/d/1zkVgI9avf_rOYFi0CtTC1xeIcKw61srKGcPENDq9_hY/edit?usp=sharing).
Just pick a row and enter your results.

    *With students, we would conduct some tactile simulations using spinners first and then use applets.*
    *But you can use R; see below for some code.*
...
# Simulate theta from the prior distribution
theta_sim = sample(seq(0.1, 0.9, 0.2), 1, replace = TRUE,
                   prob = c(1, 4, 3 * 4, 1.5 * 2 * 3 * 4, 2 * 3 * 4))

# Given theta, simulate y from a Binomial(n, theta) distribution
y_sim = rbinom(1, 25, theta_sim)
...
```{r}
#| label: fig-joint-simulation
#| ref.label: joint-simulation-plots
#| echo: false
#| fig-cap: "Results from a simulation of (theta, y) pairs" 
#| fig-subcap:
#|   - "Joint distribution of (theta, y)"
...
There are two sources or uncertainty/variability: sample-to-sample variability of $y$ ("old", "frequentist"), and uncertainty in $\theta$ represented by a probability distribution ("new", "Bayesian").
...
- Simulate a value of $\theta$ from its prior distribution (e.g., using a spinner like the one in @sec-formulating-prior that lands on 0.9 with probability 0.312, on 0.7 with probability 0.468, etc.)
(*You can use `sample` in R with the `prob` argument, but with students we would use a tactile simulation or an applet.*)
- Given $\theta$, simulate a value of $y$ from a Binomial(25, $\theta$) distribution, like we did in @sec-sampling-dist.
(*Again, students do not need Binomial distributions; they could describe a tactile simulation, or use an applet.*)
For example, if the result of the first step is $\theta = 0.5$ we would simulate $y$ from a Binomial(25, 0.5) distribution (or simulate a sample of size 25 using an applet or tactile simulation with $\theta = 0.5$).
...
# Reassessing plausibility given sample data via simulation (~5 minutes) {#sec-conditioning}
...
1. How might we use the results of the simulation in the previous section to reassess our plausibility of 0.1, 0.3, 0.5, 0.7, 0.9 as values of $\theta$ given that we observed $y=14$?
Hint: consider @fig-joint-simulation-1.
(*We start with this broad question, but students might need some additional prodding.*)


1. Use @fig-joint-simulation-1 to (roughly) rank 0.1, 0.3, 0.5, 0.7, 0.9 from most to least plausible as values of $\theta$ given that we observed $y=14$.

1. The table below summarizes the simulated $\theta$ values for repetitions with $y=14$.
Use the simulation results to rank 0.1, 0.3, 0.5, 0.7, 0.9 from most to least plausible as values of $\theta$ given that we actually observed $y=14$.
How many times more plausible is 0.7 for the value of $\theta$ than 0.5?
How could you use the simulation to approximate the **posterior distribution** of $\theta$ given that we observed $y=14$?
...
```{r}
#| ref.label: posterior-simulation
#| echo: false
...
```{r}
#| ref.label: posterior-simulation-table
#| echo: false
...
```{r}
#| ref.label: posterior-simulation-plots
#| echo: false
#| layout-ncol: 2
...
In @fig-joint-simulation-1, focus on the row of dots corresponding to $(\theta, y)$ pairs with $y = 14$.
Values of $\theta$ with relatively more dots are relatively more plausible.
Simulation suggests that given we observe $y = 14$, 0.5 and 0.7 are the most plausible values of $\theta$ (it's hard to tell which one is more plausible), followed by 0.3, while 0.1 and 0.9 have very little plausibility.
...
Since we actually observed $y=14$ we only consider simulated $(\theta, y)$ pairs for which $y=14$.
Among those pairs, `r sim_posterior_table |> filter(abs(theta_sim - 0.7) < 0.1) |> select(freq) |> pull()` correspond to $\theta = 0.7$, and `r sim_posterior_table |> filter(theta_sim == 0.5) |> select(freq) |> pull()` correspond to $\theta = 0.5$.
In this sense, given $y=14$ a value of 0.7 is `r round((sim_posterior_table |> filter(abs(theta_sim - 0.7) < 0.1) |> select(freq) |> pull()) / (sim_posterior_table |> filter(theta_sim == 0.5) |> select(freq) |> pull()), 1)` times more plausible than a value of 0.5 for $\theta$.
Among the `r sim_posterior_table |> select(freq) |> sum()` simulated $(\theta, y)$ pairs for which $y=14$, `r sim_posterior_table |> filter(abs(theta_sim - 0.7) < 0.1) |> select(freq) |> pull()` correspond to $\theta = 0.7$, so 0.7 accounts for `r 100 * round((sim_posterior_table |> filter(abs(theta_sim - 0.7) < 0.1) |> select(freq) |> pull()) / (sim_posterior_table |> select(freq) |> sum()), 3)` of the posterior plausibility.

::: {.callout-note}
We introduce Bayesian updating using words like "plausibility assessment before and after observing data" before introducing the terms "posterior distribution" or "conditional distribution of parameters given the sample data".
:::
...
```{r}
#| label: posterior-simulation-table
...
```{r}
#| label: posterior-simulation-plots
#| layout-ncol: 2
...
sim |>
  ...
  annotate("rect", xmin = 0, xmax = 1,
           ymin = y_obs - 0.4, ymax = y_obs + 0.4, alpha = 0.2,
...
sim_posterior_table |>
  ggplot(aes(x = theta_sim,
             y = rel_freq)) +
  geom_point(col = bayes_col["posterior"], size = 2) +
  geom_line(linetype = bayes_lty["posterior"], col = bayes_col["posterior"],
            linewidth = 1) +
  scale_x_continuous(limits = c(0, 1),
                     breaks = theta) +
...
# Posterior is proportional to the product of prior and likelihood (~10 minutes) {#sec-proportional}
...
*We would have students use an applet, but they can also eyeball the ratio from the sampling distributions in @fig-sampling-distributions.*)
...
1. Given relative posterior plausibilities for all possible values of $\theta$, explain how you would find the posterior distribution after observing $y=14$ with $n=25$?
(Hint: remember how we found the prior distribution from relative prior plausibilities ("units").)
...
Ratios of values in the product column provide the relative posterior probabilities.
...
::: {.callout-warning}
Prior and posterior distributions are probability distributions so they will sum (or integrate) to 1.
The likelihood function is NOT a probability distribution and will not sum (or integrate) to any particular value.
However, in order to plot prior, posterior, and likelihood on a similar scale, we often scale the likelihood to sum (or integrate) to 1.
:::
...
# A more realistic scenario (~5 minutes) {#sec-grid}

So far we have assumed that $\theta$ can only be 0.1, 0.3, 0.5, 0.7, 0.9.
But in reality $\theta$ is a proportion which could in principle take any value in [0, 1].
...
::: {.callout-note}
Since students have some familiarity with Normal distributions, we tend to use them as priors when introducing ideas.
This way, students can formulate a prior for $\theta$ by specifying a "best guess" (prior mean) and a range of values that contains "most" (say 95%) of the prior plausibility, which could then be used to identify a prior standard deviation.
:::

::: {.callout-note}
We're skipping straight to the full posterior distribution, but with students we suggest first doing a few calculations like the ones in @sec-proportional comparing prior and likelihood for values likes 0.7 and 0.5.
But if $\theta$ is a continuous random variable, the prior distribution is a density so we compare probabilities that $\theta$ is "close to" values like 0.7 and 0.5.
For example, with a Normal(0.7, 0.15) prior, it is roughly `dnorm(0.7, 0.7, 0.15) / dnorm(0.5, 0.7, 0.15)` = `r round(dnorm(0.7, 0.7, 0.15) / dnorm(0.5, 0.7, 0.15), 1)` times more plausible that $\theta$ is "close to" 0.7 than to 0.5.
:::
...
The process of computing the posterior is the same as in the previous parts; there are just a lot more rows in the Bayes table.
Some selected rows:

```{r}
#| ref.label: bayes-table-grid
#| echo: false

```

Here is a plot comparing prior, likelihood, and posterior.

```{r}
#| label: fig-posterior-grid
#| fig-cap: "Posterior distributon of theta (compared to prior and likelihood)"
#| ref.label: bayes-plot-grid
#| echo: false
...
Before observing any data, it's most plausible that around 70% of current students agree with the iterative statement.
...
The following plot displays the prior distribution, the likelihood function, and the posterior distribution.
We see that the posterior distribution is a compromise between prior distribution and the likelihood.
(Remember, the likelihood is scaled just for plotting.)


```{r}
#| label: bayes-plot-grid
...
For your reference, here is a simulation.
Now we simulate $\theta$ from its Normal prior distribution.
But given $\theta$ we simulate $y$ from a Binomial(25, $\theta$) distribution as before.


```{r}
n_rep = 100000

# Simulate values of theta from the prior distribution (truncated at 0 and 1)
theta_sim = pmax(0, pmin(1, rnorm(n_rep, 0.7, 0.15)))
...
To find the posterior distribution we condition on $y=14$ in the joint distribution.
...
sim |>
  ggplot(aes(x = theta_sim,
             y = y_sim)) +
  geom_jitter(height = 0.2, shape = 21, alpha = 0.5) +
  annotate("rect", xmin = 0, xmax = 1,
           ymin = y_obs - 0.4, ymax = y_obs + 0.4, alpha = 0.2,
           color = bayes_col["posterior"],
           fill = bayes_col["posterior"]) +
...
sim |>
  filter(y_sim == y_obs) |>
  ggplot(aes(x = theta_sim)) +
  geom_histogram(aes(y = after_stat(density)), col = bayes_col["posterior"], fill = "white") +
  geom_density(linewidth = 2, col = bayes_col["posterior"]) +
  scale_x_continuous(limits = c(0, 1)) +
  labs(title = paste("Simulated conditional distribution of theta given y = ", y_obs, ":\napproximate posterior distribution of theta"),
...
# Posterior inference (~10 minutes) {#sec-inference}


Remember that the primary goal is to make conclusions about the population based on sample data.
We can use the posterior distribution to formulate conclusions about the proportion of current [*insert your college/university here*] students who agree with the "iterative" statement.
...
1. @fig-posterior-grid displays the posterior distribution after observing the sample data ($y=14$ in a sample of $n=25$.)
...
1. @fig-posterior-grid shows that the posterior distribution of $\theta$ is approximately Normal with posterior mean `r round(posterior_mean, 3)` and posterior standard deviation `r round(posterior_sd, 3)`.
Find an interval of values that represents 68% of our posterior credibility; this is a 68% **posterior credible interval** for $\theta$.
Interpret this interval in context.


1. Compute and interpret in context a 95% **posterior credible interval** for $\theta$.

1. Compute and interpret in context the **posterior probability** that $\theta$ is greater than 0.5.
...
Here we just compute a few credible intervals, and a posterior probability.


1. The posterior distribution of $\theta$ is approximately Normal with posterior mean `r round(posterior_mean, 3)` and posterior standard deviation `r round(posterior_sd, 3)`, so we can use the 68-95 percent rule to compute credible intervals.

    A 68% posterior credible interval for $\theta$ is [`r round(posterior_mean - posterior_sd, 3)`, `r round(posterior_mean + posterior_sd, 3)`].
There is a posterior probability of 68% that between `r 100 * round(posterior_mean - posterior_sd, 3)` and `r 100 * round(posterior_mean + posterior_sd, 3)` percent of current students agree with the iterative statement.
It is roughly 2.1 times more plausible that the population proportion of current students who agree with the iterative statement lies inside the interval [`r round(posterior_mean - posterior_sd, 3)`, `r round(posterior_mean + posterior_sd, 3)`] than outside.


1. A 95% posterior credible interval for $\theta$ is [`r round(posterior_mean - 2 * posterior_sd, 3)`, `r round(posterior_mean + 2 * posterior_sd, 3)`].
There is a posterior probability of 68% that between `r 100 * round(posterior_mean - 2 * posterior_sd, 3)` and `r 100 * round(posterior_mean + 2 * posterior_sd, 3)` percent of current students agree with the iterative statement.
It is roughly 19 times more plausible that the population proportion of current students who agree with the iterative statement lies inside the interval [`r round(posterior_mean - 2 * posterior_sd, 3)`, `r round(posterior_mean + 2 * posterior_sd, 3)`] than outside.

1. The posterior probability that $\theta$ is greater than 0.5 is `r round(1 - pnorm(0.5, posterior_mean, posterior_sd), 3)`.
It is roughly `r round((1 - pnorm(0.5, posterior_mean, posterior_sd)) / pnorm(0.5, posterior_mean, posterior_sd), 1)` times more plausible than not that more than 50\% of current students agree with the iterative statement.
...
```{r}
#| ref.label: posterior-summary

```

We'll just use the fact that the posterior distribution is approximately Normal and use typical Normal distribution calculations to compute credible intervals and probabilities.

```{r}
# endpoints of central 68% posterior credible interval
posterior_mean + c(-1, 1) * posterior_sd

# endpoints of central 95% posterior credible interval
posterior_mean + 2 * c(-1, 1) * posterior_sd

# posterior probability that theta is greater than 0.5
1 - pnorm(0.5, posterior_mean, posterior_sd)
```


:::{.callout-note}
Using the grid approximation, the posterior distribution of $\theta$ is discrete so we can compute mean and standard deviation using sums.
We could also compute quantiles based on the discrete posterior, though the code is a little more complicated (see below).
While grid approximation is a good way to introduce ideas, in practice simulation is almost always used to approximate the posterior distribution, so we don't focus too much on computing things like credible intervals based on discrete posterior distributions.
:::

For your reference, here is some code that uses the discrete grid approximation directly.


```{r}
# left endpoint of central 68% posterior credible interval
min(theta[which(cumsum(posterior) >= 0.16)])

# right endpoint of central 68% posterior credible interval
min(theta[which(cumsum(posterior) >= 0.84)])

# left endpoint of central 95% posterior credible interval
min(theta[which(cumsum(posterior) >= 0.025)])

# right endpoint of central 95% posterior credible interval
min(theta[which(cumsum(posterior) >= 0.975)])
...
- We update distributions using a relatively simple formula: posterior is proportional to the product of prior and likelihood.
...
(Remember, many of the ideas introduced in this module were condensed; we would take more time and care when introducing these topics to introductory statistics students.)
...
- The prior is just one assumption of many.
Don't spend too much time on choosing the prior.
It’s the data and the posterior that matters most!
...
- [Worksheets from Kevin's STAT 415 class at Cal Poly](https://bookdown.org/kevin_davisross/stat415-handouts/)
