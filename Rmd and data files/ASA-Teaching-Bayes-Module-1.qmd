...
author: "Jingchen (Monika) Hu (Vassar) and Kevin Ross (Cal Poly)"
format:
  html:
    toc: true
...
prior_table |>
  ...
  geom_line(linetype = "dashed", col = bayes_col[1], linewidth = 1) +
...
# Reassessing plausibility (~5 minutes)
...
# Revising our simulation to reflect prior plausibility (~10 minutes)
...
sim |>
  ...
  ggplot(aes(x = theta_sim,
             y = rel_freq)) +
  geom_point(col = bayes_col[1], size = 2) +
  geom_line(linetype = bayes_lty[1], col = bayes_col[1],
            linewidth = 1) +
  scale_x_continuous(limits = c(0, 1),
                     breaks = theta) +
...
# Reassessing plausibility given sample data via simulation (~5 minutes)
...
1. Use @fig-joint-simulation (left) to (roughly) rank 0.1, 0.3, 0.5, 0.7, 0.9 from most to least plausible as values of $\theta$.
...
sim_posterior_table |>
  ggplot(aes(x = theta_sim,
             y = rel_freq)) +
  geom_point(col = bayes_col[3], size = 2) +
  geom_line(linetype = bayes_lty[3], col = bayes_col[3],
            linewidth = 1) +
  scale_x_continuous(limits = c(0, 1),
                     breaks = theta) +
...
# A more realistic scenario (~15 minutes)
...
(*We're skipping straight to the full posterior distribution, but with students we suggest first doing a few calculations like the ones comparing 0.7 and 0.5 in @sec-proportional.*)
